...
class Paginator{
    ...
    function paginate()
    {
        ...
        $this->current_page = (int) !empty($_GET['page']) ? $_GET['page'] : null; // must be numeric > 0
        ...
        if($this->num_pages > 10)
        {
            ...
            for($i=1;$i<=$this->num_pages;$i++)
            {
                ...
                {
                    $this->return .= ($i == $this->current_page And !empty($_GET['page']) ? $_GET['page'] : null != 'All') ? "<a title=\"Go to page $i of $this->num_pages\" class=\"current\" href=\"#\">$i</a> ":"<a class=\"paginate\" title=\"Go to page $i of $this->num_pages\" href=\"$_SERVER[PHP_SELF]?page=$i&ipp=$this->items_per_page\">$i</a> ";
                }
                if($this->range[$this->mid_range-1] < $this->num_pages-1 And $i == $this->range[$this->mid_range-1]) $this->return .= " ... ";
            }
            $this->return .= (($this->current_page != $this->num_pages And $this->items_total >= 10) And (!empty($_GET['page']) ? $_GET['page'] : null != 'All')) ? "<a class=\"paginate\" href=\"$_SERVER[PHP_SELF]?page=$next_page&ipp=$this->items_per_page\">Next »</a>\n":"<span class=\"inactive\" href=\"#\">» Next</span>\n";
            $this->return .= (!empty($_GET['page']) ? $_GET['page'] : null == 'All') ? "<a class=\"current\" style=\"margin-left:10px\" href=\"#\">All</a> \n":"<a class=\"paginate\" style=\"margin-left:10px\" href=\"$_SERVER[PHP_SELF]?page=1&ipp=All\">All</a> \n";
        }
        else
        {
            for($i=1;$i<=$this->num_pages;$i++)
        ...
        $this->high = ((isset($_GET['ipp']) ? $_GET['ipp'] : false) == 'All') ? $this->items_total:($this->current_page * $this->items_per_page)-1;
        $this->limit = ((isset($_GET['ipp']) ? $_GET['ipp'] : false) == 'All') ? "":" LIMIT $this->low,$this->items_per_page";
    }
