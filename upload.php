...
<input type="text" id="hornbillname" name="hornbillname">
<input type="text" id="hornbillurl" name="hornbillurl" value="https://" >
<script type="text/javascript">
  document.getElementById('hornbillname').className = "noshow";
  document.getElementById('hornbillurl').className = "noshow";
</script>
...
<!--/*End Spam bot Ankur*/-->
      ...
      <div class="grid_4 form-section">
        ...
        <fieldset>
          <label>Phone</label>
          <input type="tel" name="mobile_no" value="" id="mobile_no" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');"/>
        </fieldset>
        <?php }?>
      </div>
      ...
      <div class="grid_4 alpha form-section">
        ...
        <div id="location-type-pa" class="location-type-option active">
          ...
          <fieldset>
            ...
            <select id="location-pa1" name="location-pa">
              <option value="-1">Choose a state first</option>
              <option value="-1">Choose a Protected Area</option>
                <?php 
                $locations =  getAllPAFromStateID($con);
                foreach($locations as $location){
                  ?>
                    <option class="hide" value="<?php echo $location['PA_ID']; ?>" data-state_id = "<?php echo $location['State_Id']; ?>"><?php echo isset($location['FullName']) ? $location['FullName'] : $location['AlternativeNames']; ?></option>
                  <?php
                }
              ?>
            </select>
      ...
      <div class="grid_4 form-section">
        ...
        <fieldset>
          ...
          <select name="species" id="species">
            <option value="-1">Choose one</option>
            <?php
              $query = mysqli_query($con,'SELECT * FROM species ORDER BY species_id ASC');
        ...
        <fieldset>
          ...
          <select name="typeofincident" id="typeofincident">
            <option value="-1">Choose one</option>
            <?php
              $query = mysqli_query($con,'SELECT * FROM sight_seeing ORDER BY site_id ASC');
              while ( $row = mysqli_fetch_assoc($query) )
              {
        ...
        <fieldset>
          ...
          <input type="text" name="numbers_seen" id="numbers_seen" value=0 />
        </fieldset>
      </div>
      <div class="grid_4 omega form-section">
        ...
        <!--<div class="grid_2 omega">
          <fieldset>
            ...
            <select name="status" id="status">
              <?php /*
               $query = mysqli_query('SELECT * FROM area_status ORDER BY status_id ASC');
              while ( $row = mysqli_fetch_assoc($query) )
              {
        ...
        <div class="grid_2 alpha">
          <fieldset>
            ...
            <select name="sighting_time" id="sighting_time">
              <option value="-1">Choose One</option>
              <?php
              $query = mysqli_query($con,'SELECT * FROM sighting_time_details ORDER BY sighting_time_id ASC');
              while ( $row = mysqli_fetch_assoc($query) )
              {
        ...
        <div class="grid_2 omega">
          <fieldset>
            ...
            <select name="age" id="age">
              <option value="-1">Choose One</option>
              <?php
              $query = mysqli_query($con,'SELECT * FROM age_details ORDER BY age_id ASC');
              while ( $row = mysqli_fetch_assoc($query) )
              {
        ...
        <div class="grid_2 alpha">
          <fieldset>
            ...
            <select name="habitat" id="habitat">
              <option value="-1">Choose One</option>
              <?php
              $query = mysqli_query($con,'SELECT * FROM habitat_type ORDER BY habitat_id ASC');
              while ( $row = mysqli_fetch_assoc($query) )
              {
        ...
        <div class="grid_2 omega">
          <fieldset>
            ...
            <select name="activity" id="activity">
              <option value="-1">Choose one</option>
              <?php
              $query = mysqli_query($con,'SELECT * FROM activity ORDER BY activity_id ASC');
              while ( $row = mysqli_fetch_assoc($query) )
              {
     ...
     <input type="submit" class="btn btn-large submit" value="Submit" />
      ...
      <div class="grid_12 alpha omega">
        ...
        <span>The data generated would be summarized and shared on this website within a year.</span> </br></br>
        
     <hr class="hr" />
        <!--<p class="small-text">Please note: Specific spatial data will not be shared on the open platform but will be provided only to the IPBES project team. We understand the problems of sharing specific locations of endangered wild species. However, a rough overall map of the locations of the different species will be provided on this web page at the end of the project.</p>--> 
        <h2>download Hornbill Watch updates / reports</h2>
         <span>
               <a href="https://hornbills.in/hornbill-watch-report-july-2020.pdf" target="_blank" style="color:#fff;">Hornbill Watch report July 2020</a></br>
               <a href="https://hornbills.in/hornbill-watch-update-2017.pdf" target="_blank" style="color:#fff;">Hornbill watch update 2017</a></br>
               <a href="https://hornbills.in/hornbill-watch-update-sept-2015.pdf" target="_blank" style="color:#fff;">Hornbill Watch Update Sept 2015</a></br>
          <a href="https://hornbills.in/hornbill-watch-update-3rd-june-2014-12-may-2015.pdf" target="_blank" style="color:#fff;">Hornbill watch update 3rd June 2014 to 12 May 2015</a></br>
          <a href="https://hornbills.in/hornbill-watch-website-summary-june-december-2014.pdf" target="_blank" style="color:#fff;">Hornbill watch website summary June to December 2014</a>
</span>
        </div>
</div>
